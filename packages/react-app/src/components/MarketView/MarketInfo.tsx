...
import {UseLiquidityPool, useLiquidityPool} from "../../hooks/useLiquidityPool";
...
function MarketPrizeInfo({liquidityPool, market}: {liquidityPool: UseLiquidityPool, market: Market}) {
  if (!liquidityPool) {
    return <>
  ...
  return <>
    <Typography variant="p3" component="div"><Trans id="Full Prize" /></Typography>
    <Typography variant="h5" component="h5">{formatAmount(liquidityPool.prizePool.add(minPrize))}</Typography>
    <Typography variant="p3" component="div" sx={{mt: 2}}>Minimum Prize</Typography>
    <Typography variant="h5" component="h5">{formatAmount(minPrize)}</Typography>
  </>
}
...
function MarketInfo({market}: {market: Market}) {
  ...
  return <GridStyled container spacing={0}>
    ...
    <Grid item xs={6} md={3}>
      ...
      <Typography variant="h6s" component="h6">{(managementFee + market.protocolFee) * 100 / DIVISOR}%</Typography>
      <div style={{fontSize: '11.11px'}}>{(market.protocolFee * 100) / DIVISOR}% protocol + {(managementFee * 100) / DIVISOR}% manager</div>
      {liquidityPool && <>
          <Typography variant="p3" component="div" sx={{mt: 2}}><Trans id="Total Liquidity" /></Typography>
    ...
    <Grid item xs={6} md={3}>
      ...
      <Typography variant="h6s" component="h6">
        <a href={`https://blockscout.com/xdai/mainnet/address/${manager}/transactions`} target="_blank" rel="noreferrer">
