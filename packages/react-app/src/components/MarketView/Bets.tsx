    ...
    <div>
      ...
      {ranking && ranking.length > 0 && ranking.map((rank, i) => {
        ...
        return <TableBody key={i}>
          ...
          <div style={{width: '40%'}}><Link to={`/profile/${rank.player.id}`}>{(address && rank.player.id.toLowerCase() === address.toLowerCase()) ? i18n._("You") : formatPlayerName(rank.player.name, rank.player.id)}</Link></div>
