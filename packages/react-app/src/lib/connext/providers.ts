...
import { delay } from '@/lib/connext/axios'
import { domainToChainId } from '@/lib/connext/domain'
import { NxtpError, parseError, RpcError, ServerError, StallTimeout } from '@/lib/connext/errors'
...
export class SyncProvider extends StaticJsonRpcProvider {
	...
	constructor(
		_connectionInfo: ConnectionInfo | string,
		...
		this.name = parseHostname(this.connectionInfo.url)
			? parseHostname(this.connectionInfo.url)!
					.split('.')
					.slice(0, -1)
					.join('.')
