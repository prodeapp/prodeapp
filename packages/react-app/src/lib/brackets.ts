...
export const getEliminationConfig = (
	...
	while (accumEvents <= totalEvents) {
		config.groups.unshift({
			size: currentEvents,
			name: mainRoundNames[n] || `${altRoundNames.replace('%', String(currentEvents * 2))}`,
		})
...
export const getDoubleEliminationConfig = (events: Event[]): EliminationConfig[] => {
	...
	brackets.push({
		...
		config: getEliminationConfig(
			totalTeams - 1,
			['Winners Final', 'Winners Semifinals', 'Winners Quarterfinals'],
			'Winners Round of %'
		),
	})
	...
	// final match 1
	brackets.push({
		questions: eventsCopy.splice(0, 1),
		config: getEliminationConfig(1, [], 'Final #1'),
	})
	...
	const losersConfig = getEliminationConfig(
		totalLosersTeams,
		['Losers Final #1', 'Losers Semifinals #1', 'Losers Quarterfinals #1'],
		'Losers Round of % #1'
	)
	...
	brackets.push({ questions: eventsCopy, config: losersConfig })
...
export const parseEliminationConfig = (events: Event[], config: ExtraDataGroups): ParsedEliminationConfig[] => {
	...
	return config.sizes.map((size, i) => {
		return {
			events: Array.from({ length: size }, (_, i) => i + 1).map(_ => {
				const n = t++
				return events[n]
			}),
			name: config.names[i] || '',
...
export const getDoubleEliminationMatches = (events: Event[]): DoubleElimLeaderboardProps['matches'] => {
	const doubleEliminationConfig = getDoubleEliminationConfig(events).map(data =>
		parseEliminationConfig(data.questions, convertExtraDataGroups(data.config))
	)

	const matches: DoubleElimLeaderboardProps['matches'] = {
		upper: [],
		lower: [],
	}
...
export const getSingleEliminationMatches = (events: Event[]): Match[] => {
	const bracketConfig = parseEliminationConfig(
		events,
		convertExtraDataGroups(getEliminationConfig(events.length, ['Final', 'Semifinals', 'Quarterfinals'], '', true))
	)
